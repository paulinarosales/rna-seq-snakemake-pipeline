...
qval_th <- params$qval_th_cluster
...
for(i in 1:nrow(degs_sum)){
  ...
  ego_mol <- enrichGO(gene = names(geneList),
                OrgDb = org.Ce.eg.db,
                keyType = "ENSEMBL",
                ont = "MF",
                pAdjustMethod = "BH",
                pvalueCutoff  = padj_th_cluster,
                qvalueCutoff  = qval_th)
  
  ego_biop <- enrichGO(gene = names(geneList),
                OrgDb = org.Ce.eg.db,
                keyType = "ENSEMBL",
                ont = "BP",
                pAdjustMethod = "BH",
  ...
  if (nrow(ego_mol) > 0){
    p_goeMol <- dotplot(ego_mol, showCategory=10) +
                  labs(title = paste("Molecular function GOE for", degs_sum$Contrast[i])) +
                  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10), 
                        legend.text = element_text(size=7), legend.title = element_text(size=8),legend.key.size = unit(0.5, "cm")) + 
                    xlim(c(0,0.125)) 

    p_goeBiop <- dotplot(ego_biop, showCategory=10) +
                  labs(title = paste("Biological process GOE for", degs_sum$Contrast[i])) +
                  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10), 
                        legend.text = element_text(size=7), legend.title = element_text(size=8),legend.key.size = unit(0.5, "cm")) + 
                    xlim(c(0,0.125)) 

    p_goeHist <- mutate(ego_mol, qscore = -log(p.adjust, base=10)) %>% 
                    barplot(x="qscore")
    ...
    cat("No enriched terms found for this contrast.")
  }
