...
pawlette <- pawlette[1:unique(sample_manifest$Sample_type)]
dual_col <- c("#CAB2D6", "#6A3D9A") # purple shades

blue_col <- brewer.pal(9, "Blues")
red_col <- brewer.pal(9, "Reds")

pca_col <- scale_colour_manual(name = "Sample_type", values=pawlette) # PCA

ann_col <- list(Sample_type = c(pawlette)) # pheatmap
ann_col <- pawlette
...
freq_all %>% filter(raw_counts > 0) %>% ggplot(aes(x = sample, y = log2cpm, fill=target, alpha=treatment)) + 
                        geom_boxplot() + scale_fill_manual(values=pawlette) +
...
writeLines(c(paste("## Date:\t", format(Sys.time(), '%d %B, %Y')),
            ...
            "## CPM counts where calculated using `edgeR`"
